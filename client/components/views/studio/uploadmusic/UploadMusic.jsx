...
const UploadMusic = () => {
  ...
  return (
    <form onSubmit={minting} style={style.uploadContainer}>
      {
        s3 && s3.MintMusicNFTLoading && <UploadingMusic /> // Uploading
      }
      {
        s3.MintMusicNFTDone && <UploadEnd /> // After upload is finished
      }
      {(s3.S3UploadError || s3.IPFSUploadError || s3.MintMusicNFTError) && (
        <UploadError /> // When an error occurs during upload
      )}
